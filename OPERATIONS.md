...
`healthz` и `readyz` выполняют быструю проверку доступности Django и БД.
Полный SQLite `integrity_check` выполняют резервное копирование и команда
`check_operational_health`, чтобы публичная readiness-проба не блокировала
рабочие запросы. `/ops/status/` и `/ops/notifications/` доступны только
глобальному администратору: первая точка возвращает JSON и SHA релиза, вторая
позволяет повторить или закрыть исчерпанные уведомления.
...
Проверьте `docker compose logs deadline-worker`, `/ops/status/` и
`/ops/notifications/`. После
устранения сетевой ошибки записи со статусом `failed`, не исчерпавшие число
попыток, будут отправлены повторно. Исчерпанные записи можно повторно поставить
в очередь или закрыть через интерфейс глобального администратора.
